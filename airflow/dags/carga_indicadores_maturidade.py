"""
DAG: carga_indicadores_maturidade

Busca os dados de maturidade na API FATIMA/PETIC e grava direto nas
tabelas finais `ouro.fato_maturidade` e `ouro.fato_maturidade_pilares`.
"""
import os
...
# Só precisa garantir o schema "ouro" e as tabelas de maturidade em ouro.
ARQUIVOS_ESTRUTURA = [
    "schema_ouro.sql",
...
def carregar_e_publicar_ouro():
    """
    Busca os dados na API, monta os DataFrames finais em memória e grava
    direto nas tabelas ouro (substituindo o conteúdo antigo com
    DELETE + INSERT, já que não existe mais staging em bronze).
    """
    engine = conectar()
        ...
        [
            {
                ...
                "pontos_faltantes": dados["pontos_possiveis"] - dados["pontos_obtidos"],
    ...
    df_org["ultima_data_avaliacao"] = pd.to_datetime(
        df_org["ultima_data_avaliacao"], errors="raise",
    )
    ...
    with engine.begin() as conn:
        ...
        df_org.to_sql("fato_maturidade", conn, schema="ouro", if_exists="append", index=False)
        df_pilares.to_sql("fato_maturidade_pilares", conn, schema="ouro", if_exists="append", index=False)

    print(
        f"Ouro atualizada: 1 órgão, {len(df_pilares)} pilares gravados. "
...
with DAG(
    ...
    tarefa_ouro = PythonOperator(
        task_id="carregar_e_publicar_ouro",
        python_callable=carregar_e_publicar_ouro,
    )
